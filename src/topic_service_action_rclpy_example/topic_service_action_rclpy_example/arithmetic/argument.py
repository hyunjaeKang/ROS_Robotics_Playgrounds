...
class Argument(Node):
    ...
    def update_parameter(self, params):
        # self.get_logger().info("Callback function (update_parameter) ---")
        for param in params:
            if param.name == 'min_random_num' and param.type == Parameter.Type.INTEGER:
                self.min_random_num = param.value
                self.get_logger().info(f'min_random_num is changed to {self.min_random_num}')
            elif param.name == 'max_random_num' and param.type == Parameter.Type.INTEGER:
                self.max_random_num = param.value
                self.get_logger().info(f'max_random_num is changed to {self.max_random_num}')
