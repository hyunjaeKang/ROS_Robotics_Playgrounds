#include "calculator/calcultor.hpp"
  ...
  const int8_t & operators)
{
  ...
  if(operators == arithmetic_operator.PLUS){
    ...
    if (b == 0){
      ...
      argument_result = 0.0;      return argument_result;
    }
    argument_result = a / b;
  }else{
    RCLCPP_ERROR(
...
{
  RCLCPP_INFO(
    this->get_logger(), "Received request to cancel goal");
  (void)goal_handle;
  ...
  const std::shared_ptr<GoalHandleArithmeticChecker> goal_handle)
{
  ...
  while((total_sum < goal_sum) && rclcpp::ok()){
    ...
    if(argument_formula_.empty()){
      RCLCPP_WARN(this->get_logger(), "Please check you formula");
      break;
    }
